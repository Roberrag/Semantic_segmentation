...
def test_hook_default(
    ...
    for i, sample in enumerate(iterator):
        ...
        if get_key_metric is not None:
            status = status + ", Metric_avg: {0:.5}".format(metric_fn.get_metric_value()[0])
        iterator.set_description(status)
    output = {"metric": metric_fn.get_metric_value()[0], "loss": loss_avg.avg}
    countflag = 0
    for item in metric_fn.get_metric_value()[1]:
        wandb.log({"Dice_{}".format(countflag): item})
        countflag += 1
    return output
...
def end_epoch_hook_semseg(iterator, epoch, output_train, output_test):
    ...
    if hasattr(iterator, "set_description"):
        iterator.set_description(
            "epoch: {0}, test_dice: {1:.5}, train_loss: {2:.5}, test_loss: {3:.5}".format(
                epoch, output_test["metric"], output_train["loss"], output_test["loss"]
            )
        )

    if hasattr(iterator, "set_extra_description"):
        iterator.set_extra_description("test_dices", output_test["metric"])
